...
path = "C:\\Users\\allen\\Desktop\\Esther\\dl\\mlp\\";
model_path = "C:\\Users\\allen\\Desktop\\Esther\\dl\\mlp\\model\\";
trian_path = "C:\\Users\\allen\\Desktop\\Esther\\dl\\Training\\all\\mlp\\";
test_path = "C:\\Users\\allen\\Desktop\\Esther\\dl\\Testing\\mlp\\";


# filep=open(path+'dl_mlp_result.txt',"a")
...
# filep.close()
# filep_2=open(path+'dl_mlp_result_score.txt',"a")
# filep_2.write('MHC\tans\tpredict\tscore\n')
# filep_2.close()
for t in range(36,len(MHC_dl),2):
    print(t)
    ...
    #model.add(tf.keras.layers.Dense(1024, use_bias=True,activation='relu',input_shape=(x.shape[1],)))
    #model.add(tf.keras.layers.Dropout(0.1))
    #model.add(tf.keras.layers.Dense(num_classes, activation='softmax'))
    model.add(tf.keras.layers.Dense(400, use_bias=True,activation='relu',input_shape=(x.shape[1],)))
    model.add(tf.keras.layers.Dropout(0.2))
    model.add(tf.keras.layers.Dense(200,use_bias=True, activation='relu'))
    model.add(tf.keras.layers.Dropout(0.2))
...
#    shuffle=True,
    ...
    # model = tf.keras.models.load_model(model_path+file_name+'_mlp.h5')
    ...
    for num in range(0,len(resultsClass2)):
        if test_x2[num][0] == 1.0 and test_x2[num][1] == 0.0 and resultsClass2[num] == 0:
            ...
            filep_2=open(path+'dl_mlp_result_score.txt',"a")
            filep_2.write(MHC_dl[t]+"\t"+'0'+'\t'+'0'+'\t'+str(results[num][0])+'\t'+str(results[num][1])+'\n')
            filep_2.close()
            ...
            pred.append(results[num][1])
            filep_2=open(path+'dl_mlp_result_score.txt',"a")
            filep_2.write(MHC_dl[t]+"\t"+'0'+'\t'+'1'+'\t'+str(results[num][0])+'\t'+str(results[num][1])+'\n')
            filep_2.close()
            ...
            pred.append(results[num][1])
            filep_2=open(path+'dl_mlp_result_score.txt',"a")
            filep_2.write(MHC_dl[t]+"\t"+'1'+'\t'+'1'+'\t'+str(results[num][0])+'\t'+str(results[num][1])+'\n')
            filep_2.close()
            ...
            pred.append(results[num][1])
            filep_2=open(path+'dl_mlp_result_score.txt',"a")
            filep_2.write(MHC_dl[t]+"\t"+'1'+'\t'+'0'+'\t'+str(results[num][0])+'\t'+str(results[num][1])+'\n')
            filep_2.close()
    # print(pred)
    # results_sort = pred.sort()
    # print(results_sort)
    score = model.evaluate(test_x, test_x2, verbose=0)
    filep=open(path+'dl_mlp_result.txt',"a")
    filep.write(MHC_dl[t]+"\t"+str(round(score[0],4))+"\t"+str(round(score[1],4))+"\t"+str(auc(y_ans,pred))+"\t"+str(mcc(tp,tn,fp,fn))+"\t"+str(f1(tp,fn,fp))+"\t"+str(sp(tn,fp))+"\t"+str(sn(tp,fn))+"\t"+str(tp)+"\t"+str(fn)+"\t"+str(tn)+"\t"+str(fp)+"\n")
